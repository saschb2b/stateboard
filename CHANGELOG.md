...
- **Board settings, with a Danger Zone.** A gear in the editor opens per-board settings (`/boards/[id]/settings`): rename the board and edit its description from a General section, and — in a GitHub-style Danger Zone — delete the board behind a type-the-name confirmation. Deleting removes its screens, regions, share links, and image files.
