...
- **Board settings.** A gear in the editor opens per-board settings (`/boards/[id]/settings`) with a section sidebar. **General** renames the board, edits its description, and — in a GitHub-style Danger Zone — deletes the board behind a type-the-name confirmation (which removes its screens, regions, share links, and image files). **Sharing** mints public links named per audience, copies them, and revokes any one without affecting the others.
