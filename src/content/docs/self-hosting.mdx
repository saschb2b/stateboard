...
<Callout type="warn">
  ...
  `base64` password — whose `/` is reserved in a URL — produces a cryptic
  `Invalid URL` at startup. A non-URL-safe password must be percent-encoded. If
  you bring your own `externalDatabaseUrl`, encode the password there too.
</Callout>
